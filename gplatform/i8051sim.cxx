...
//--------- cosimulation control interface
#include "rtsim.h"
extern rtsimgen *glbRTsimgen;

extern void i8051externalwrite(int dev, unsigned char d);

//-----------------------------------------------------------------------------

i8051sim::i8051sim() : InvalidData(11),
                 LineLength(80),
                 RecordTypeLength(3),
                 ACC(0x0160),
                 PSW(0x0150),
...
bool i8051sim::IsRunning() {
  return !progEnd;
}


void i8051sim::ClockTick() {
  targetCycleCount++;

  if (targetCycleCount > cycleCount) {
    // may simulate next clock cycle

    SimulateInstruction();

    if (lastP0 != RAM[P0]) {
      if (external_decoded_addr.find(P0) != external_decoded_addr.end())
	i8051externalwrite(external_decoded_addr[P0], RAM[P0]);
    }
    if (lastP1 != RAM[P1]) {
      if (external_decoded_addr.find(P1) != external_decoded_addr.end())
	i8051externalwrite(external_decoded_addr[P1], RAM[P1]);
    }
    if (lastP2 != RAM[P2]) {
      if (external_decoded_addr.find(P2) != external_decoded_addr.end())
	i8051externalwrite(external_decoded_addr[P2], RAM[P2]);
    }
    if (lastP3 != RAM[P3]) {
      if (external_decoded_addr.find(P3) != external_decoded_addr.end())
	i8051externalwrite(external_decoded_addr[P3], RAM[P3]);
    }
...
void i8051sim::SimulateInstruction() {

#ifdef DEBUG
#ifdef DEBUG_PC
	  cerr << setw(5) << setfill('0') << PC << " - ";	       
#endif
#endif
	  ...
	  // record P0-P3 to detect a write
	  lastP0 = RAM[P0];
	  lastP1 = RAM[P1];
	  lastP2 = RAM[P2];
	  lastP3 = RAM[P3];
	  ...
	  case ANL2:
	    ...
	    cerr << "ANL 2" << endl;
#ifdef DETAIL
	    cerr << "\t" << "A <- A & RAM(" << (unsigned int)(unsigned char)ROM[PC] << ")" << endl;
#endif
#endif
	    IR = ROM[PC++];
	  ...
	  case ANL3:
	    ...
	    cerr << "ANL 3" << endl;
#ifdef DETAIL
	    cerr << "\t" << "A <- A & RAM(R" << (int)(IR & 0x01) << ")" << endl;
#endif
#endif
	    regNum = IR & 0x01;
	  ...
	  case ANL4:
	    ...
	    cerr << "ANL 4" << endl;
#ifdef DETAIL
	    cerr << "\t" << "A <- A & "; PrintHex(ROM[PC], &os); cerr << endl;
#endif
#endif
	    IR = ROM[PC++];
	    RAM[ACC] &= (char)IR;
	  ...
	  case ANL5:
	    ...
	    cerr << "ANL 5" << endl;
#ifdef DETAIL
	    cerr << "\t" << "RAM(" << (unsigned int)(unsigned char)ROM[PC] 
	  ...
	  case ANL6:
	    ...
	    cerr << "\t" << "RAM(" << (unsigned int)(unsigned char)ROM[PC] 
	       << ") <- RAM(" << (unsigned int)(unsigned char)ROM[PC] 
	       << " & "; 
	    PrintHex(ROM[PC+1], &os); cerr << endl;
#endif
#endif
	    directAddr = ROM[PC++];
	    IR = ROM[PC++];
	  ...
	  case XCH2:
	    ...
	    cerr << "XCH 2" << endl;
#ifdef DETAIL
	    cerr << "\t" << "A <-> RAM(" << (unsigned int)(unsigned char)(ROM[PC]) << ")" << endl;
#endif
#endif
	    IR = ROM[PC++];
	    temp = RAM[ACC];
	  ...
	  case XCH3:
	    ...
	    cerr << "XCH 3" << endl;
#ifdef DETAIL
	    cerr << "\t" << "A <-> RAM(R" << (int)(IR & 0x01) << ")" << endl;
#endif
#endif
	    regNum = IR & 0x01;
	    temp = RAM[ACC];
	    RAM[ACC] = RAM[(unsigned) RAM[GetRegisterBank()+regNum]];
	    RAM[(unsigned) RAM[GetRegisterBank()+regNum]] = temp;
	    cycleCount += 12;
	    break;
	  ...
	  case XCHD:
	    ...
	    cerr << "XCHD" << endl;
#ifdef DETAIL
	    cerr << "\t" << "A.(3-0) <-> RAM(R" << (int)(IR & 0x01) << ").(3-0)" << endl;
#endif
#endif
	    regNum = IR & 0x01;
	    temp = RAM[ACC] & 0x0F;
	    RAM[ACC] = (RAM[ACC] & 0xF0) | 
	      (RAM[(unsigned) RAM[GetRegisterBank()+regNum]] & 0x0F);
	    RAM[(unsigned) RAM[GetRegisterBank()+regNum]] = 
	      (RAM[(unsigned) RAM[GetRegisterBank()+regNum]] & 0xF0 ) | temp;
	    cycleCount += 12;
	    break;
	  ...
	  case CJNE1:
	    ...
	    cerr << "CJNE 1" << endl;
#ifdef DETAIL
	    cerr << "\t" << "if( A != RAM(" << (unsigned int)(unsigned char)ROM[PC] 
	  ...
	  case CJNE2:
	    ...
	    cerr << "\t" << "if( A != ";
	    PrintHex(ROM[PC], &os);
	    cerr << " ) JMP " << (PC+(char)ROM[PC+1]+2) << endl
	       << "\t" << "if( A < ";
	    PrintHex(ROM[PC], &os);
	    cerr << " ) C <- 1" << endl
	       << "\t" << "else C <- 0" << endl;
	  ...
	  case CJNE4:
	    ...
	    IR = ROM[PC++];
	    directAddr = IR;
	    if( RAM[(unsigned) RAM[GetRegisterBank()+regNum]] != (char)IR ) {
	      IR = ROM[PC++];
	      PC += (char)IR;
	    }
	    else {
	    ...
	    if( (unsigned char)RAM[(unsigned) RAM[GetRegisterBank()+regNum]] < 
		(unsigned char)directAddr ) {
	      SetBit(RAM[PSW], CY);
	    }
	    else {
	  ...
	  case DEC3:
	    ...
	    cerr << "DEC 3" << endl;
#ifdef DETAIL
	    cerr << "\t" << "RAM(" << (unsigned int)(unsigned char)ROM[PC] << ")--" << endl; 
#endif
#endif
	    IR = ROM[PC++];
	  ...
	  case DJNZ2:
	    ...
	    cerr << "\t" << "if( RAM(" << (unsigned int)(unsigned char)ROM[PC] 
	       << ") != 0 ) JMP " 
	  ...
	  case MOV8:
	    ...
	    cerr << "MOV 8" << endl;
#ifdef DETAIL
	    cerr << "\t" << "RAM(" << (unsigned int)(unsigned char)(ROM[PC]) << ") <- A" << endl;
#endif
#endif
	    IR = ROM[PC++];
	    RAM[(IR < 128) ? IR : (IR+128)] = RAM[ACC];
	  ...
	  case MOV9:
	    ...
	    cerr << "MOV 9" << endl;
#ifdef DETAIL
	    cerr << "\t" << "RAM(" << (unsigned int)(unsigned char)(ROM[PC]) << ") <- R" 
	       << (int)(IR & 0x07) << endl;
#endif
#endif
	    regNum = IR & 0x07;
	  ...
	  case MOV10:
	    ...
	    cerr << "MOV 10" << endl;
#ifdef DETAIL
	    cerr << "\t" << "RAM(" << (unsigned int)(ROM[PC+1]) << ") <- RAM(" 
	       << (unsigned int)(unsigned char)(ROM[PC]) << ")" << endl;
#endif
#endif
	    directAddr = ROM[PC++];
	  ...
	  case MOV11:
	    ...
	    cerr << "MOV 11" << endl;
#ifdef DETAIL
	    cerr << "\t" << "RAM(" << (unsigned int)(unsigned char)(ROM[PC]) << ") <- RAM(R" 
	       << (int)(IR & 0x01) << ")" << endl;
#endif
#endif
	    regNum = IR & 0x01;
	  ...
	  case MOV13:
	    ...
	    cerr << "MOV 13" << endl;
#ifdef DETAIL
	    cerr << "\t" << "RAM(R" << (int)(IR & 0x01) << ") <- A" << endl;
#endif
#endif
	    regNum = IR & 0x01;
	    RAM[(unsigned) RAM[GetRegisterBank()+regNum]] = RAM[ACC];
	    cycleCount += 12;
	    break;
	  ...
	  case MOV15:
	    ...
	    regNum = IR & 0x01;
	    IR = ROM[PC++];;
	    RAM[(unsigned) RAM[GetRegisterBank()+regNum]] = (char)IR;
	    cycleCount += 12;
	    break;
	  ...
	  case MOVX1:		    
	    ...
	    cerr << "MOVX 1" << endl;
#ifdef DETAIL
	    cerr << "\t" << "A <- XRAM(R" << (int)(IR & 0x01) << ")" << endl;
#endif
#endif
	    regNum = IR & 0x01;
	    RAM[ACC] = readXRAM((unsigned) RAM[GetRegisterBank()+regNum]);
	    cycleCount += 24;
	    break;
	  ...
	  case MOVX2:
	    ...
	    cerr << "MOVX 2" << endl;
#ifdef DETAIL
	    cerr << "\t" << "A <- XRAM(" << tempDPTR << ")" << endl;
#endif
#endif
	    RAM[ACC] = readXRAM(tempDPTR);
	    cycleCount += 24;
	    break;
	  ...
	  case MOVX3:
	    ...
	    cerr << "MOVX 3" << endl;
#ifdef DETAIL
	    cerr << "\t" << "XRAM(R" << (int)(IR & 0x01) << ") <- A" << endl;
#endif
#endif
	    regNum = IR & 0x01;
	    { 
	      int v = (unsigned) RAM[GetRegisterBank()+regNum];

	      writeXRAM(v, RAM[ACC]);

	      if (external_decoded_addr.find(v) != external_decoded_addr.end())
		i8051externalwrite(external_decoded_addr[v], RAM[ACC]);
	    }
	    cycleCount += 24;
	    break;
	  ...
	  case MOVX4:
	    ...
	    cerr << "MOVX 4" << endl;
#ifdef DETAIL
	    cerr << "\t" << "XRAM(" << tempDPTR << ") <- A" << endl;
#endif
#endif

	    writeXRAM(tempDPTR, RAM[ACC]);
	    if (external_decoded_addr.find(tempDPTR) != external_decoded_addr.end())
	      i8051externalwrite(external_decoded_addr[tempDPTR], RAM[ACC]);
	  ...
	  case ADD1:
...
#ifdef DETAIL
	    cerr << "\t" << "A <- A + R" << (int)(IR & 0x07) << endl;
	    cerr << "\t" << "A = "; PrintHex(RAM[ACC], &os); cerr << endl;
#endif
#endif
	    carry3 = false;
	    carry6 = false;
	    ...
	    PrintHex(RAM[ACC], &os); 
	    cerr << endl;
	    cerr << carry3 << " " << carry6 << " " << carry7 << endl;
#endif
#endif
	  ...
	  case ADD3:
...
#ifdef DETAIL
	    cerr << "\t" << "B = ";
	    PrintHex(RAM[(unsigned) RAM[GetRegisterBank()+regNum]], &os); 
	    cerr << endl;
#endif
#endif
	    tempAdd = (RAM[ACC] & 0x0F) + 
	    ...
	    if( (tempAdd & 0x0010) == 0x0010 ) carry3 = true;
	    tempAdd += ((RAM[ACC] & 0x70) + 
			(RAM[(unsigned) RAM[GetRegisterBank()+regNum]] & 0x70));
	    if( (tempAdd & 0x0080) == 0x0080 ) carry6 = true;
	    tempAdd += ((RAM[ACC] & 0x80) + 
			(RAM[(unsigned) RAM[GetRegisterBank()+regNum]] & 0x80));
	    if( (tempAdd & 0x0100) == 0x0100 ) carry7 = true;
	    RAM[ACC] = tempAdd;
	    if( carry3 ) SetBit(RAM[PSW], AC);
	    else ClearBit(RAM[PSW], AC);
	  ...
	  case ADDC2:
	    ...
	    cerr << "ADDC 2" << endl;
#ifdef DETAIL
	    cerr << "\t" << "A <- A + RAM(" << (unsigned int)(unsigned char)(ROM[PC]) << ") + C" << endl;
	    cerr << "\t" << "A = "; PrintHex(RAM[ACC], &os); cerr << endl;
#endif
#endif
	    carry3 = false;
	  ...
	  case ADDC3:
	    ...
	    cerr << "ADDC 3" << endl;
#ifdef DETAIL
	    cerr << "\t" << "A <- A + RAM(R" << (int)(IR & 0x01) << ") + C" << endl;
#endif
#endif
	    carry3 = false;
	    carry6 = false;
	  ...
	  case SUBB2:
	    ...
	    PrintHex(RAM[ACC], &os); 
	    cerr << endl;
	    cerr << borrow3 << " " << borrow6 << " " << borrow7 << endl;
#endif
#endif
	    break;
...
#endif
	    if( (unsigned char)(RAM[ACC] & 0x0F) <
		(unsigned char)((RAM[(unsigned) RAM[GetRegisterBank()+regNum]] & 0x0F) + 
				(char)GetBit(RAM[PSW], CY)) ) {
	      borrow3 = true;
	    }
	    if( (unsigned char)(RAM[ACC] & 0x7F) <
		(unsigned char)((RAM[(unsigned) RAM[GetRegisterBank()+regNum]] & 0x7F) + 
				(char)GetBit(RAM[PSW], CY)) ) {
	      borrow6 = true;
	    }
	    if( (unsigned short)(unsigned char)RAM[ACC] <
		((unsigned short)(unsigned char)RAM[(unsigned) RAM[GetRegisterBank()+regNum]] + 
		 (unsigned short)GetBit(RAM[PSW], CY)) ) {
	      borrow7 = true;
	    }
	    RAM[ACC] = (unsigned short)(unsigned char)RAM[ACC] - 
...
//-----------------------------------------------------------------------------
int i8051sim::register_addr(unsigned short ad) {
  static int uniq_id = 0;
  return (external_decoded_addr[ad] = uniq_id++);
}

//-----------------------------------------------------------------------------

bool
i8051sim::ProgramCompletion() 
{
...
i8051sim::Init8051() {
  ...
  if (external_decoded_addr.find(P0) != external_decoded_addr.end())
    i8051externalwrite(external_decoded_addr[P0], 0xFF);

  if (external_decoded_addr.find(P1) != external_decoded_addr.end())
    i8051externalwrite(external_decoded_addr[P1], 0xFF);

  if (external_decoded_addr.find(P2) != external_decoded_addr.end())
    i8051externalwrite(external_decoded_addr[P2], 0xFF);

  if (external_decoded_addr.find(P3) != external_decoded_addr.end())
    i8051externalwrite(external_decoded_addr[P3], 0xFF);
...
Opcode 
i8051sim::Decode(const unsigned char IR) 
{    
  // BBBBBBBB
  switch( IR ) {
  case 0x25:
...
void 
i8051sim::SetBit(char &thisByte, unsigned char thisBit) 
{    
...
void 
i8051sim::ClearBit(char &thisByte, unsigned char thisBit) 
{
  thisByte &= ~(0x01 << thisBit);
}
...
unsigned char
i8051sim::GetBit(char thisByte, unsigned char thisBit) 
{
...
void
i8051sim::PrintHex(unsigned char byte, ostream* os) 
{
...
void i8051sim::writeRAM(unsigned int a, unsigned char d)  {
  if (a < RamSize) {
    RAM[a] = d;
  }
}

void i8051sim::writeXRAM(unsigned int a, unsigned char d)  {
  // cerr << "Write XRAM @ " << a << " - d " << (unsigned int) d << "\n";
  if (a < XRamSize) {
    XRAM[a] = d;
  }
}

unsigned char i8051sim::readXRAM(unsigned int a)  {
  // cerr << "Read XRAM @ " << a << " - d " << (unsigned int) XRAM[a] << "\n";
  if (a < XRamSize) {
    return XRAM[a];
  }
  return 0;
...
bool
i8051sim::Hex2Short(const char* buf, unsigned &val) 
{    
  int i;
...
bool 
i8051sim::Load(const char* buf, unsigned char* rom, unsigned& prgSize) 
{   
  unsigned temp;
...
bool
i8051sim::LoadHex(const char *filename) 
{
